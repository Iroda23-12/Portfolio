import React from 'react';
import './Dior.css'; // Убедитесь, что путь к CSS файлу правильный

const Dior = () => {
  return (
    <header>
      <div className="menyu">
        <div className="container">
          <div className="rasm">
            <ul>
              <li>FASHION & ACCESSORIES FRAGRANCE & BEAUTY</li>
            </ul>
            <h1>DIOR</h1>
          </div>
        </div>
      </div>

      {/* pasdigi sozlar */}
      <div className="end-menu">
        <br />
        <p>MAKEUP</p>
      </div>

      <div className="menu1">
        <h3 className="text-wrap text-break u-margin-bottom--xxs u-text-title-l">
          ROUGE DIOR
        </h3>
        <a className="button button--secondary" aria-label="Discover ROUGE DIOR" href="#" target="_self">
          Discover
        </a>
      </div>

      {/* 2pastdagi menu */}
      <div className="rasm2">
        <img
          src="https://www.dior.com/on/demandware.static/-/Library-Sites-DiorSharedLibrary/default/dw09ae364d/images/beauty/0-HOME/FRAGRANCES/2024/4-APRIL/A24_F095_LCP-CRUISE_GammeSansBoite_5550x2000_LCP.jpg"
          alt=""
        />
      </div>

      {/* pasidigi sozlar */}
      <div className="div">
        <br />
        <p>FRAGRANCE</p>
        <h3>LA COLLECTION PRIVÉE CHRISTIAN DIOR</h3>
        <a className="button button--secondary" aria-label="Discover ROUGE DIOR" href="#" target="_self">
          Discover
        </a>
      </div>

      {/* 3pasidagi menu */}
      <div className="rasm3">
        <img
          src="https://www.dior.com/on/demandware.static/-/Library-Sites-DiorSharedLibrary/default/dw477fb1df/images/beauty/0-HOME/SKINCARE/2024/1_JANUARY/CT_Hyalushot24_Mood_Solo_Texture_5550X2000.jpg"
          alt=""
        />
      </div>

      {/* 3pasidagi sozlar */}
      <div className="po">
        <br />
        <p>SKINCARE</p>
        <h3>HYALUSHOT</h3>
        <a className="button button--secondary" aria-label="Discover ROUGE DIOR" href="#" target="_self">
          Discover
        </a>
      </div>

      {/* 4pasidagi menu */}
      <div className="rasm4">
        <img
          src="https://www.dior.com/on/demandware.static/-/Library-Sites-DiorSharedLibrary/default/dw360d3f6e/images/beauty/0-HOME/BEAUTY/2024/4-APRIL/Adorables_Gamme_5550X2000_WFRAG.jpg"
          alt=""
        />
      </div>

      <div className="op">
        <br />
        <p>FRAGRANCE</p>
        <h3>J'ADORE</h3>
        <a className="button button--secondary" aria-label="Discover ROUGE DIOR" href="#" target="_self">
          Discover
        </a>
      </div>

      {/* 5pasidagi menu */}
      <div className="rasm5">
        <img
          src="https://www.dior.com/on/demandware.static/-/Library-Sites-DiorSharedLibrary/default/dw7242fcac/images/beauty/0-HOME/FRAGRANCES/2024/1_JANUARY/DH24_Ecom_Instit_Intense_5550x2000.jpg"
          alt=""
        />
      </div>

      <div className="lo">
        <br />
        <p>FRAGRANCE</p>
        <h3>DIOR HOMME</h3>
        <a className="button button--secondary" aria-label="Discover ROUGE DIOR" href="#" target="_self">
          Discover
        </a>
      </div>

      {/* pasidagi sozlar */}
      <div className="end-nav">
        <ul>
          <li>
            <a href="#"><b>EXPLORE MORE</b></a>
          </li>
          <li><a href="#">Dior Bath & Body Soap</a></li>
          <li><a href="#">Nail Polish Lacquers</a></li>
          <li><a href="#">La Collection Privée</a></li>
          <li><a href="#">Rouge Dior</a></li>
        </ul>
      </div>

      {/* end nav */}
      <div className="idish">
        <div className="uy">
          {/* Уберите закомментированный код, если он не нужен */}
          <img src="photo_2024-05-31_11-52-59.jpg" width="350px" height="300px" alt="" />
          <img src="photo_2024-05-31_11-22-34.jpg" width="300px" height="450px" alt="" />
          <img src="photo_2024-05-31_11-26-06.jpg" width="350px" height="390px" alt="" />
        </div>
      </div>
    </header>
  );
};

export default Dior;
